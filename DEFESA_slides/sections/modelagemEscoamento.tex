...
\begin{frame}{Modelagem de escoamentos monofásicos}
  Quando se aplica uma pressão (ou fluxo) em um domínio saturado por apenas \textbf{um fluido}, é induzido o que se chama de \textbf{escoamento monofásico}.

  O escoamento de um fluido em um meio poroso é o \textit{fluxo que ocorre nos espaços vazios interconectados do meio}. A conservação de massa do fluido implica que, dado um volume $V$, a massa acumulada em $V$ deve ser igual a taxa de fluido pelas bordas, mais a injeção de massa:
...
\begin{frame}{Modelagem de escoamentos monofásicos}
  ...
  \end{equation}
  onde $c_t = c_f + c_r$ representa a compressibilidade total. Considerado um \textbf{escoamento incompressível} (ou seja, quando a rocha e o fluido são incompressíveis), $\rho$ e $\phi$ são independentes de $p$ e, portanto, $c_t = 0$. A equação (\theequation)\ torna-se uma equação elíptica com coeficientes variáveis:
...
\begin{frame}{Condições auxiliares}
  ...
  onde $u$ é uma função escalar, como por exemplo a temperatura. Estas condições podem ser \textbf{homogêneas}, quando o contorno é igual a zero, ou \textbf{heterogêneas}, quando são iguais a uma função $g$.
\end{frame}
...
\begin{frame}{Meios homogêneos e heterogêneos}
  ...
  \begin{itemize}
    \item Um meio dito \textbf{homogêneo}, governado pelo sistema de escoamentos monofásicos (\theequation), ocorre quando a permeabilidade absoluta $K$ é constante e uniforme, logo não dependendo de $x$; ou
